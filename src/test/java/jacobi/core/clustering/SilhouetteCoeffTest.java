...
@JacobiImport("/jacobi/test/data/SilhouetteCoeffTest.xlsx")
@RunWith(JacobiJUnit4ClassRunner.class)
public class SilhouetteCoeffTest {
	...
	public void shouldBeAbleToComputeSilhouetteInGauss5D4CentroidsWith2Clusters() {
		...
		double sil = new SilhouetteCoeff().applyAsDouble(input, clusters);
	...
	@Test
	@JacobiImport("test rand 2-D 2 centroids")
	@JacobiEquals(expected = 10, actual = 10)
	public void shouldBeAbleToComputeSilhouetteInRand2D2Centroids() {
		List<int[]> clusters = this.toClusters(this.clusterMat);
		double sil = new SilhouetteCoeff().applyAsDouble(input, clusters);
	...
	@Test
	@JacobiImport("test rand 2-D 2 centroids (3)")
	@JacobiEquals(expected = 10, actual = 10)
	public void shouldBeAbleToComputeSilhouetteInRand2D2CentroidsWith3Clusters() {
		List<int[]> clusters = this.toClusters(this.clusterMat);
		double sil = new SilhouetteCoeff().applyAsDouble(input, clusters);
